...
# Updated sections with your specific full-text questions
sections = [
...
for title, qs in sections:
    ...
    for code, label in qs:
        # index=None forces the user to pick an option (Mandatory)
    ...
    # Justification keys remain 13, 15, 17, 19
    j_key = str(int(code[:2]) + 1) 
    responses[j_key] = st.text_area(
        f"Justification ({title})", 
        value=prev_resp.get(j_key, ""), 
        disabled=disabled, 
        key=f"j{j_key}",
        placeholder="Optional: Provide reasoning for the section above..."
    )
    st.divider()
...
j21 = st.text_area(
    "Final justification", 
    value=prev_data.get('overall_justification', ""), 
    disabled=disabled,
    placeholder="Provide your overall concluding remarks here..."
)
